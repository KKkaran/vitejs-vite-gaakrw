...
function App() {
  ...
  const createPara = () => {
    ...
    return wordsOfPara.map((w, index) => {
      if (
        styleWords['red'].includes(index) &&
        styleWords['bold'].includes(index) &&
        styleWords['underline'].includes(index)
      )
        return (
          <span
            style={{
      ...
      if (
        ...
        return <span style={{ color: 'red', fontWeight: 'bold' }}>{w} </span>;
      if (
        styleWords['red'].includes(index) &&
        styleWords['underline'].includes(index)
      )
      ...
      if (styleWords['bold'].includes(index))
        return <span style={{ fontWeight: 'bold' }}>{w} </span>;
      if (styleWords['red'].includes(index))
        return <span style={{ color: 'red' }}>{w} </span>;
      if (styleWords['underline'].includes(index))
        return <span style={{ textDecoration: 'underline' }}>{w} </span>;
      return <span>{w} </span>;
    });
  };
  return (
    <div className="App bg-green-900 p-4 rounded-xl">
      <div>
      <span style={{textDecoration:"underline"}}>Underline</span>: <input className='p-1 rounded-xl'
          type="text"
        ...
        />
        <br /><br />
        <span style={{fontWeight:"Bold"}}>Bold</span>: <input className='p-1 rounded-xl'
          type="text"
        ...
        />
        <br /><br />
        <span style={{color:"red"}}>Red Color</span>:<input
          type="text" className='p-1 rounded-xl'
